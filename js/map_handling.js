...
function locateCallback(location){
	if (location === -1){
...
function updateMarkers(xml) {
	...
	if ($("status",xml).text() != "success") {
		...
		for (var i = 0; i < storeList.length; i++) {
			...
			var marker;
			if (map.getZoom() > MIN_DETAIL_ZOOM){
				var html = infoWindowHtml(name, street, houseNr, zip, city, supplier, imageUrl);
...
infoWindowHtml = function(name, street, houseNr, zip, city, supplier, imageUrl) {
