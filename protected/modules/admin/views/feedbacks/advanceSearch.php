...
$simpleSearch = array('search');
if (isset(Yii::app()->session['Feedbacks']) && isset(Yii::app()->session['Feedbacks']['time'])){
	$simpleSearch=array_merge($simpleSearch,array('newSearch'=>Yii::app()->session['Feedbacks']['time']));
}

if ($this->isFancyAjaxRequest){ ?>
	<input type="hidden" id="FancyChooseSubmitLink" value="<?php echo $this->createUrl('advanceChooseFeedbacks'); ?>"/>
	<?php
}
?>

<div id="feedbacksAdvanceSearch">
...
/*	
	...
	<div class="row">
		<?php echo $form->label($model,'FEE_ID'); ?>
	...
	<div class="row">
		<?php echo $form->label($model,'FEE_STATUS'); ?>
		<?php echo $form->textField($model,'FEE_STATUS'); ?>
	</div>
