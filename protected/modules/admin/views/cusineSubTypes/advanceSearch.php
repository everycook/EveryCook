...
if (isset(Yii::app()->session['CusineSubTypes']) && isset(Yii::app()->session['CusineSubTypes']['time'])){
	$advanceSearch=array_merge($advanceSearch,array('newSearch'=>Yii::app()->session['CusineSubTypes']['time']));
}

if ($this->isFancyAjaxRequest){ ?>
	<input type="hidden" id="FancyChooseSubmitLink" value="<?php echo $this->createUrl('advanceChooseCusineSubTypes'); ?>"/>
	<?php
}
?>

<div id="cusine-sub-typesAdvanceSearch">
...
/*	
	...
	<div class="row">
		<?php echo $form->label($model,'CST_GPS_LAT'); ?>
		<?php echo $form->textField($model,'CST_GPS_LAT'); ?>
	</div>

	<div class="row">
		<?php echo $form->label($model,'CST_GPS_LNG'); ?>
		<?php echo $form->textField($model,'CST_GPS_LNG'); ?>
	</div>

	<div class="row">
		<?php echo $form->label($model,'CST_GOOGLE_REGION'); ?>
	...
	<div class="row">
		<?php echo $form->label($model,'CST_DESC_DE_CH'); ?>
		<?php echo $form->textField($model,'CST_DESC_DE_CH',array('size'=>60,'maxlength'=>100)); ?>
	</div>
