...
*/
?>
<input type="hidden" id="uploadImageLink" value="<?php echo $this->createUrl('uploadImage',array('id'=>$model->CST_ID)); ?>"/>
...
<?php $form=$this->beginWidget('CActiveForm', array(
	...
	?>
	<?php
		if (isset(Yii::app()->session[$this->createBackup]) && isset(Yii::app()->session[$this->createBackup]->CST_IMG_ETAG)){
			echo CHtml::image($this->createUrl('displaySavedImage', array('id'=>'backup', 'ext'=>'.png', 'rand'=>rand())), '', array('class'=>'ingredient'));
		} else if ($model->CST_ID && isset($model->CST_IMG_ETAG)) {
			echo CHtml::image($this->createUrl('displaySavedImage', array('id'=>$model->CST_ID, 'ext'=>'.png')), '', array('class'=>'ingredient'));
		}
	?>
	<div class="row">
