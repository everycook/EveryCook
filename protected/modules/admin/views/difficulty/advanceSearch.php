...
if (isset(Yii::app()->session['Difficulty']) && isset(Yii::app()->session['Difficulty']['time'])){
	$advanceSearch=array_merge($advanceSearch,array('newSearch'=>Yii::app()->session['Difficulty']['time']));
}

if ($this->isFancyAjaxRequest){ ?>
	<input type="hidden" id="FancyChooseSubmitLink" value="<?php echo $this->createUrl('advanceChooseDifficulty'); ?>"/>
	<?php
}
?>

<div id="difficultyAdvanceSearch">
...
/*	
	...
	<div class="row">
		<?php echo $form->label($model,'DIF_ID'); ?>
		<?php echo $form->textField($model,'DIF_ID'); ?>
	</div>

	<div class="row">
		<?php echo $form->label($model,'DIF_ORDER'); ?>
		<?php echo $form->textField($model,'DIF_ORDER'); ?>
	</div>

	<div class="row">
		<?php echo $form->label($model,'DIF_DESC_EN_GB'); ?>
		<?php echo $form->textField($model,'DIF_DESC_EN_GB',array('size'=>60,'maxlength'=>100)); ?>
	</div>

	<div class="row">
		<?php echo $form->label($model,'DIF_DESC_DE_CH'); ?>
