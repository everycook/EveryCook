...
class FoodAllergyController extends Controller
{
	...
	protected $createBackup = 'FoodAllergy_Backup';
	protected $searchBackup = 'FoodAllergy';
	...
	public function accessRules(){
		return array(
			array('allow',  // allow all users to perform 'index' and 'view' actions
				'actions'=>array('index','view','search','advanceSearch','chooseFoodAllergy','advanceChooseFoodAllergy'),
				'users'=>array('*'),
			),
	...
	private function getModel($id){
		...
		if (isset($id) && $id != null){
			if (!isset($oldmodel) || $oldmodel->FAL_ID != $id){
	...
	private function prepareCreateOrUpdate($id, $view){
		...
		{
			$model->attributes=$_POST['FoodAllergy'];
			if($model->save()){
				unset(Yii::app()->session[$this->createBackup]);
				unset(Yii::app()->session[$this->createBackup.'_Time']);
				$this->forwardAfterSave(array('view', 'id'=>$model->FAL_ID));
				//$this->forwardAfterSave(array('search', 'query'=>$model->__get('FAL_DESC_' . Yii::app()->session['lang'])));
				return;
			}
		}
	...
	public function actionIndex(){
		$dataProvider=new CActiveDataProvider('FoodAllergy');
	...
	private function prepareSearch($view, $ajaxLayout, $criteria){
		...
		if (isset($Session_Data)){
			if(!isset($_POST['SimpleSearchForm']) && !isset($_GET['query']) && !isset($_POST['FoodAllergy']) && (!isset($_GET['newSearch']) || $_GET['newSearch'] < $Session_Data['time'])){
	...
	public function actionChooseFoodAllergy(){
		$this->isFancyAjaxRequest = true;
		$this->prepareSearch('search', 'none', null);
	}
	
	public function actionAdvanceChooseFoodAllergy(){
		$this->isFancyAjaxRequest = true;
	...
	public function actionShowLike(){
		...
		$criteria->compare(FoodAllergy::model()->tableName().'.FAL_ID',$ids);
	...
	public function loadModel($id){
		if ($id == 'backup'){
			...
			$model=FoodAllergy::model()->findByPk($id);
		}
		if($model===null)
	...
	protected function performAjaxValidation($model){
		if(isset($_POST['ajax']) && $_POST['ajax']==='food-allergy-form')
		{
