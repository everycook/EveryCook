...
class ActionsInController extends Controller
{
	/**
	...
	protected $createBackup = 'ActionsIn_Backup';
	protected $searchBackup = 'ActionsIn';
	...
	public function accessRules(){
		return array(
			array('allow',  // allow all users to perform 'index' and 'view' actions
				'actions'=>array('index','view','search','advanceSearch','chooseActionsIn','advanceChooseActionsIn'),
				'users'=>array('*'),
			),
	...
	private function getModel($id){
		...
		if (isset($id) && $id != null){
			if (!isset($oldmodel) || $oldmodel->AIN_ID != $id){
	...
	private function prepareCreateOrUpdate($id, $view){
		...
		if(isset($_POST['ActionsIn']))
		{
			$model->attributes=$_POST['ActionsIn'];
			if($model->save()){
				unset(Yii::app()->session[$this->createBackup]);
				unset(Yii::app()->session[$this->createBackup.'_Time']);
				$this->forwardAfterSave(array('view', 'id'=>$model->AIN_ID));
				//$this->forwardAfterSave(array('search', 'query'=>$model->__get('AIN_DESC_' . Yii::app()->session['lang'])));
				return;
			}
		}
	...
	public function actionIndex(){
		$dataProvider=new CActiveDataProvider('ActionsIn');
	...
	private function prepareSearch($view, $ajaxLayout, $criteria){
		...
		if (isset($Session_Data)){
			if(!isset($_POST['SimpleSearchForm']) && !isset($_GET['query']) && !isset($_POST['ActionsIn']) && (!isset($_GET['newSearch']) || $_GET['newSearch'] < $Session_Data['time'])){
	...
	public function actionChooseActionsIn(){
		$this->isFancyAjaxRequest = true;
		$this->prepareSearch('search', 'none', null);
	}
	
	public function actionAdvanceChooseActionsIn(){
		$this->isFancyAjaxRequest = true;
	...
	public function actionShowLike(){
		...
		$criteria->compare(ActionsIn::model()->tableName().'.AIN_ID',$ids);
	...
	public function loadModel($id){
		if ($id == 'backup'){
			...
			$model=ActionsIn::model()->findByPk($id);
		}
		if($model===null)
	...
	protected function performAjaxValidation($model){
		if(isset($_POST['ajax']) && $_POST['ajax']==='actions-in-form')
		{
