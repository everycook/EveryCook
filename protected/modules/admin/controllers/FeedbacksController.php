...
class FeedbacksController extends Controller
{
	...
	protected $createBackup = 'Feedbacks_Backup';
	protected $searchBackup = 'Feedbacks';
	...
	private function getModel($id){
		...
		if (isset($id) && $id != null){
			if (!isset($oldmodel) || $oldmodel->FEE_ID != $id){
	...
	private function prepareCreateOrUpdate($id, $view){
		...
		if(isset($_POST['Feedbacks']))
		{
			...
			if($model->save()){
				unset(Yii::app()->session[$this->createBackup]);
				unset(Yii::app()->session[$this->createBackup.'_Time']);
				$this->forwardAfterSave(array('view', 'id'=>$model->FEE_ID));
				//$this->forwardAfterSave(array('search', 'query'=>$model->__get('FEE_DESC_' . Yii::app()->session['lang'])));
				return;
			}
		}
	...
	private function prepareSearch($view, $ajaxLayout, $criteria){
		...
		if (isset($Session_Data)){
			if(!isset($_POST['SimpleSearchForm']) && !isset($_GET['query']) && !isset($_POST['Feedbacks']) && (!isset($_GET['newSearch']) || $_GET['newSearch'] < $Session_Data['time'])){
	...
	protected function performAjaxValidation($model){
		if(isset($_POST['ajax']) && $_POST['ajax']==='feedbacks_form')
		{
