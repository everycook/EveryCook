...
class StepTypesController extends Controller
{
	/**
	...
	protected $createBackup = 'StepTypes_Backup';
	protected $searchBackup = 'StepTypes';
	...
	public function accessRules(){
		return array(
			array('allow',  // allow all users to perform 'index' and 'view' actions
				'actions'=>array('index','view','search','advanceSearch','chooseStepTypes','advanceChooseStepTypes'),
				'users'=>array('*'),
			),
	...
	private function getModel($id){
		...
		if (isset($id) && $id != null){
			if (!isset($oldmodel) || $oldmodel->STT_ID != $id){
	...
	private function prepareCreateOrUpdate($id, $view){
		...
		if(isset($_POST['StepTypes']))
		{
			$model->attributes=$_POST['StepTypes'];
			if($model->save()){
				unset(Yii::app()->session[$this->createBackup]);
				unset(Yii::app()->session[$this->createBackup.'_Time']);
				$this->forwardAfterSave(array('view', 'id'=>$model->STT_ID));
				//$this->forwardAfterSave(array('search', 'query'=>$model->__get('STT_DESC_' . Yii::app()->session['lang'])));
				return;
			}
		}
	...
	private function prepareSearch($view, $ajaxLayout, $criteria){
		...
		if (isset($Session_Data)){
			if(!isset($_POST['SimpleSearchForm']) && !isset($_GET['query']) && !isset($_POST['StepTypes']) && (!isset($_GET['newSearch']) || $_GET['newSearch'] < $Session_Data['time'])){
	...
	public function actionChooseStepTypes(){
		$this->isFancyAjaxRequest = true;
		$this->prepareSearch('search', 'none', null);
	}
	
	public function actionAdvanceChooseStepTypes(){
		$this->isFancyAjaxRequest = true;
	...
	public function actionShowLike(){
		...
		$criteria->compare(StepTypes::model()->tableName().'.STT_ID',$ids);
	...
	public function loadModel($id){
		if ($id == 'backup'){
			...
			$model=StepTypes::model()->findByPk($id);
		}
		if($model===null)
	...
	protected function performAjaxValidation($model){
		if(isset($_POST['ajax']) && $_POST['ajax']==='step-types-form')
		{
