...
class ConditionsController extends Controller
{
	...
	protected $createBackup = 'Conditions_Backup';
	protected $searchBackup = 'Conditions';
	...
	public function accessRules(){
		return array(
			array('allow',  // allow all users to perform 'index' and 'view' actions
				'actions'=>array('index','view','search','advanceSearch','chooseConditions','advanceChooseConditions'),
				'users'=>array('*'),
			),
	...
	private function getModel($id){
		...
		if (isset($id) && $id != null){
			if (!isset($oldmodel) || $oldmodel->CND_ID != $id){
	...
	private function prepareCreateOrUpdate($id, $view){
		...
		if(isset($_POST['Conditions']))
		{
			...
			if($model->save()){
				unset(Yii::app()->session[$this->createBackup]);
				unset(Yii::app()->session[$this->createBackup.'_Time']);
				$this->forwardAfterSave(array('view', 'id'=>$model->CND_ID));
				//$this->forwardAfterSave(array('search', 'query'=>$model->__get('CND_DESC_' . Yii::app()->session['lang'])));
				return;
			}
		}
	...
	private function prepareSearch($view, $ajaxLayout, $criteria){
		...
		if (isset($Session_Data)){
			if(!isset($_POST['SimpleSearchForm']) && !isset($_GET['query']) && !isset($_POST['Conditions']) && (!isset($_GET['newSearch']) || $_GET['newSearch'] < $Session_Data['time'])){
	...
	public function actionChooseConditions(){
		$this->isFancyAjaxRequest = true;
		$this->prepareSearch('search', 'none', null);
	}
	
	public function actionAdvanceChooseConditions(){
		$this->isFancyAjaxRequest = true;
	...
	public function actionShowLike(){
		...
		$criteria->compare(Conditions::model()->tableName().'.CND_ID',$ids);
