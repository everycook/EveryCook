...
class RelationTypsController extends Controller
{
	/**
	...
	protected $createBackup = 'RelationTyps_Backup';
	protected $searchBackup = 'RelationTyps';
	...
	public function accessRules(){
		return array(
			array('allow',  // allow all users to perform 'index' and 'view' actions
				'actions'=>array('index','view','search','advanceSearch','chooseRelationTyps','advanceChooseRelationTyps'),
				'users'=>array('*'),
			),
	...
	private function getModel($id){
		...
		if (isset($id) && $id != null){
			if (!isset($oldmodel) || $oldmodel->REL_ID != $id){
	...
	private function prepareCreateOrUpdate($id, $view){
		...
		if(isset($_POST['RelationTyps']))
		{
			$model->attributes=$_POST['RelationTyps'];
			if($model->save()){
				unset(Yii::app()->session[$this->createBackup]);
				unset(Yii::app()->session[$this->createBackup.'_Time']);
				$this->forwardAfterSave(array('view', 'id'=>$model->REL_ID));
				//$this->forwardAfterSave(array('search', 'query'=>$model->__get('REL_DESC_' . Yii::app()->session['lang'])));
				return;
			}
		}
	...
	public function actionIndex(){
		$dataProvider=new CActiveDataProvider('RelationTyps');
	...
	private function prepareSearch($view, $ajaxLayout, $criteria){
		...
		if (isset($Session_Data)){
			if(!isset($_POST['SimpleSearchForm']) && !isset($_GET['query']) && !isset($_POST['RelationTyps']) && (!isset($_GET['newSearch']) || $_GET['newSearch'] < $Session_Data['time'])){
	...
	public function actionChooseRelationTyps(){
		$this->isFancyAjaxRequest = true;
		$this->prepareSearch('search', 'none', null);
	}
	
	public function actionAdvanceChooseRelationTyps(){
		$this->isFancyAjaxRequest = true;
	...
	public function actionShowLike(){
		...
		$criteria->compare(RelationTyps::model()->tableName().'.REL_ID',$ids);
	...
	public function loadModel($id){
		if ($id == 'backup'){
			...
			$model=RelationTyps::model()->findByPk($id);
		}
		if($model===null)
	...
	protected function performAjaxValidation($model){
		if(isset($_POST['ajax']) && $_POST['ajax']==='relation-typs-form')
		{
