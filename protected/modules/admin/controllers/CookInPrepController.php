...
class CookInPrepController extends Controller
{
	/**
	...
	protected $createBackup = 'CookInPrep_Backup';
	protected $searchBackup = 'CookInPrep';
	...
	public function accessRules(){
		return array(
			array('allow',  // allow all users to perform 'index' and 'view' actions
				'actions'=>array('index','view','search','advanceSearch','chooseCookInPrep','advanceChooseCookInPrep'),
				'users'=>array('*'),
			),
	...
	private function getModel($id){
		...
		if (isset($id) && $id != null){
			if (!isset($oldmodel) || $oldmodel->COI_PREP != $id){
	...
	private function prepareCreateOrUpdate($id, $view){
		...
		if(isset($_POST['CookInPrep']))
		{
			$model->attributes=$_POST['CookInPrep'];
			if($model->save()){
				unset(Yii::app()->session[$this->createBackup]);
				unset(Yii::app()->session[$this->createBackup.'_Time']);
				$this->forwardAfterSave(array('view', 'id'=>$model->COI_PREP));
				//$this->forwardAfterSave(array('search', 'query'=>$model->__get('COI_DESC_' . Yii::app()->session['lang'])));
				return;
			}
		}
	...
	public function actionIndex(){
		$dataProvider=new CActiveDataProvider('CookInPrep');
	...
	private function prepareSearch($view, $ajaxLayout, $criteria){
		...
		if (isset($Session_Data)){
			if(!isset($_POST['SimpleSearchForm']) && !isset($_GET['query']) && !isset($_POST['CookInPrep']) && (!isset($_GET['newSearch']) || $_GET['newSearch'] < $Session_Data['time'])){
	...
	public function actionChooseCookInPrep(){
		$this->isFancyAjaxRequest = true;
		$this->prepareSearch('search', 'none', null);
	}
	
	public function actionAdvanceChooseCookInPrep(){
		$this->isFancyAjaxRequest = true;
	...
	public function actionShowLike(){
		...
		$criteria->compare(CookInPrep::model()->tableName().'.COI_PREP',$ids);
	...
	public function loadModel($id){
		if ($id == 'backup'){
			...
			$model=CookInPrep::model()->findByPk($id);
		}
		if($model===null)
	...
	protected function performAjaxValidation($model){
		if(isset($_POST['ajax']) && $_POST['ajax']==='cook-in-prep-form')
		{
