...
class CookingStoveController extends Controller
{
	/**
	...
	protected $createBackup = 'CookingStove_Backup';
	protected $searchBackup = 'CookingStove';
	...
	public function accessRules(){
		return array(
			array('allow',  // allow all users to perform 'index' and 'view' actions
				'actions'=>array('index','view','search','advanceSearch','chooseCookingStove','advanceChooseCookingStove'),
				'users'=>array('*'),
			),
	...
	private function getModel($id){
		...
		if (isset($id) && $id != null){
			if (!isset($oldmodel) || $oldmodel->COS_ID != $id){
	...
	private function prepareCreateOrUpdate($id, $view){
		...
		if(isset($_POST['CookingStove']))
		{
			$model->attributes=$_POST['CookingStove'];
			if($model->save()){
				unset(Yii::app()->session[$this->createBackup]);
				unset(Yii::app()->session[$this->createBackup.'_Time']);
				$this->forwardAfterSave(array('view', 'id'=>$model->COS_ID));
				//$this->forwardAfterSave(array('search', 'query'=>$model->__get('COS_DESC_' . Yii::app()->session['lang'])));
				return;
			}
		}
	...
	public function actionIndex(){
		$dataProvider=new CActiveDataProvider('CookingStove');
	...
	private function prepareSearch($view, $ajaxLayout, $criteria){
		...
		if (isset($Session_Data)){
			if(!isset($_POST['SimpleSearchForm']) && !isset($_GET['query']) && !isset($_POST['CookingStove']) && (!isset($_GET['newSearch']) || $_GET['newSearch'] < $Session_Data['time'])){
	...
	public function actionChooseCookingStove(){
		$this->isFancyAjaxRequest = true;
		$this->prepareSearch('search', 'none', null);
	}
	
	public function actionAdvanceChooseCookingStove(){
		$this->isFancyAjaxRequest = true;
	...
	public function actionShowLike(){
		...
		$criteria->compare(CookingStove::model()->tableName().'.COS_ID',$ids);
	...
	public function loadModel($id){
		if ($id == 'backup'){
			...
			$model=CookingStove::model()->findByPk($id);
		}
		if($model===null)
	...
	protected function performAjaxValidation($model){
		if(isset($_POST['ajax']) && $_POST['ajax']==='cooking-stove-form')
		{
