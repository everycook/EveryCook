...
class CookInController extends Controller
{
	/**
	...
	protected $createBackup = 'CookIn_Backup';
	protected $searchBackup = 'CookIn';
	...
	public function accessRules(){
		return array(
			array('allow',  // allow all users to perform 'index' and 'view' actions
				'actions'=>array('index','view','search','advanceSearch','chooseCookIn','advanceChooseCookIn'),
				'users'=>array('*'),
			),
	...
	private function getModel($id){
		...
		if (isset($id) && $id != null){
			if (!isset($oldmodel) || $oldmodel->COI_ID != $id){
	...
	private function prepareCreateOrUpdate($id, $view){
		...
		if(isset($_POST['CookIn']))
		{
			$model->attributes=$_POST['CookIn'];
			if($model->save()){
				unset(Yii::app()->session[$this->createBackup]);
				unset(Yii::app()->session[$this->createBackup.'_Time']);
				$this->forwardAfterSave(array('view', 'id'=>$model->COI_ID));
	...
	public function actionIndex(){
		$dataProvider=new CActiveDataProvider('CookIn');
	...
	private function prepareSearch($view, $ajaxLayout, $criteria){
		...
		if (isset($Session_Data)){
			if(!isset($_POST['SimpleSearchForm']) && !isset($_GET['query']) && !isset($_POST['CookIn']) && (!isset($_GET['newSearch']) || $_GET['newSearch'] < $Session_Data['time'])){
	...
	public function actionChooseCookIn(){
		$this->isFancyAjaxRequest = true;
		$this->prepareSearch('search', 'none', null);
	}
	
	public function actionAdvanceChooseCookIn(){
		$this->isFancyAjaxRequest = true;
	...
	public function actionShowLike(){
		...
		$criteria->compare(CookIn::model()->tableName().'.COI_ID',$ids);
	...
	public function loadModel($id){
		if ($id == 'backup'){
			...
			$model=CookIn::model()->findByPk($id);
		}
		if($model===null)
	...
	protected function performAjaxValidation($model){
		if(isset($_POST['ajax']) && $_POST['ajax']==='cook-in-form')
		{
