...
class CaptchaExtendedAction extends CCaptchaAction{
   ...
   public $mode = self::MODE_MATHVERBAL;
	...
	*/
	protected function getCodeLogical(){
		...
		$a = array();
		for($i=0;$i<$t;++$i){
			// we dont use zero & we don't want equal numbers
         $s = mt_rand(1,9);
         while(in_array($s, $a))
            $s = mt_rand(1,9);
			$a[] = mt_rand(1,9);
	...
	*/
	protected function getCodeMathVerbal(){
      // check for not too long Captchas
      $check = false;
		...
		switch ($check ? mt_rand(0,1) : mt_rand(0,2)){
			case 0:
			...
			case 1:
				...
				break;
			case 2:
				$question = Yii::t('main','What\'s the result for');
				break;
		}
